...
from app_runtime import ensure_parent_dir, get_bundle_dir, get_runtime_dir, resolve_input_path, resolve_output_path

BUNDLE_DIR_ENV = "OPENAI_REGISTER_BUNDLE_DIR"
RUNTIME_DIR_ENV = "OPENAI_REGISTER_RUNTIME_DIR"
APP_NAME = "OpenAI Register Full"
...
def _resolve_input_path(path: str) -> str:
    return str(resolve_input_path(path, APP_NAME))
...
VERIFY_API_URL_BY_DOMAIN = {
    "usncf.xyz": VERIFY_API_URL,
    "usapiops.com": "https://codexworker.lhl9532g.workers.dev/",
    "codexrun.site": "https://workingrot.triplew150.workers.dev/",
}
EMAIL_DOMAINS = tuple(VERIFY_API_URL_BY_DOMAIN.keys())
DOMAIN = "auto"
PROXY = _CONFIG["proxy"]
...
def _page_type_from_payload(payload: Any) -> str:
    return _as_str(_as_dict(_as_dict(payload).get("page")).get("type"))


def _orgs_from_payload(payload: Any) -> list[dict[str, Any]]:
    orgs_value = _as_dict(_as_dict(payload).get("data")).get("orgs", [])
    if not isinstance(orgs_value, list):
        return []
    return [item for item in orgs_value if isinstance(item, dict)]
...
_IMPERSONATE_TARGETS = [
    cast(str, profile["impersonate"])
    for profile in _CHROME_PROFILES
    if isinstance(profile.get("impersonate"), str)
]
_LANGUAGE_PROFILES = [
...
# ================= 邮箱相关 =================
def _resolve_verify_api_url(email_or_domain=None):
    value = str(email_or_domain or "").strip().lower()
    if "@" in value:
        _, _, value = value.rpartition("@")
    if value in {"", "auto", "random"}:
        value = random.choice(EMAIL_DOMAINS) if EMAIL_DOMAINS else "usncf.xyz"
    domain = value or str(_current_domain or DOMAIN).strip().lower()
    return VERIFY_API_URL_BY_DOMAIN.get(domain, VERIFY_API_URL)


def _resolve_email_domain(domain=None):
    value = str(domain if domain is not None else _current_domain).strip().lower()
    if value in {"", "auto", "random"}:
        return random.choice(EMAIL_DOMAINS) if EMAIL_DOMAINS else "usncf.xyz"
    return value


def _normalize_verification_code(raw):
    digits = re.sub(r"\D", "", str(raw or ""))
    return digits[:6] if len(digits) >= 6 else None


def _extract_verification_code_from_text(text):
    blob = str(text or "")
    if not blob:
        return None
    patterns = [
    ...
    for pattern in patterns:
        ...
        if not match:
            continue
        code = _normalize_verification_code(match.group(1))
        if code:
            return code
    return None


def _extract_verification_code_from_payload(payload):
    if payload is None:
        return None
    if isinstance(payload, (str, int, float)):
        return _extract_verification_code_from_text(str(payload))
    if isinstance(payload, dict):
        for key in ("code", "otp", "verification_code", "verificationCode"):
            code = _normalize_verification_code(payload.get(key))
            if code:
                return code
        for key in ("subject", "text", "html", "content", "raw", "body", "message"):
            code = _extract_verification_code_from_payload(payload.get(key))
            if code:
                return code
        for key in ("data", "result", "payload", "mail", "email", "messages", "emails", "items", "list"):
            code = _extract_verification_code_from_payload(payload.get(key))
            if code:
                return code
        for value in payload.values():
            code = _extract_verification_code_from_payload(value)
            if code:
                return code
        return None
    if isinstance(payload, (list, tuple)):
        for item in reversed(payload):
            code = _extract_verification_code_from_payload(item)
            if code:
                return code
    return None
...
def generate_email():
    ...
    return f"{prefix}@{_resolve_email_domain()}"
...
def wait_code(email, max_wait=120, interval=3):
    ...
    verify_api_url = _resolve_verify_api_url(email_key)
        ...
        try:
            ...
            if r.status_code == 200:
                try:
                    ...
                    payload = None
                code = _extract_verification_code_from_payload(payload)
                if not code:
                    code = _extract_verification_code_from_text(r.text)
                if code:
                    print(f" -> {code}")
                    return code
...
class OpenAIRegister:
    ...
    def _oauth_allow_redirect_extract_code(self, url: str, referer: Optional[str] = None):
    ...
    def _oauth_submit_workspace_and_org(self, consent_url: str):
        ...
        orgs = _orgs_from_payload(ws_data)
    ...
    def perform_codex_oauth_login_http(self, email: str, password: str, name: str = "OpenAI User", birthdate: str = "1995-01-01"):
        ...
        page_type = _page_type_from_payload(continue_data)
        ...
        page_type = _page_type_from_payload(verify_data) or page_type
        ...
        if need_oauth_otp:
            ...
            while time.time() < otp_deadline and not otp_success:
                ...
                for otp_code in candidate_codes:
                    ...
                    page_type = _page_type_from_payload(otp_data) or page_type
                    otp_success = True
                    break
                ...
                try:
                    ...
                    if resp_ay.status_code == 200:
                        try:
                            ...
                            new_page = _page_type_from_payload(ay_d)
...
class SyncedOpenAIRegister(OpenAIRegister):
    ...
    def _create_browser_session(self):
        session = _new_http_session(self.impersonate)
        _apply_session_proxies(session, _proxies())
    ...
    def _session_get_with_retry(self, url, *, step, max_attempts=3, **kwargs):
        ...
        if last_exc is None:
            raise RuntimeError(f"[{step}] session retry exited without a response or exception")
        raise last_exc
            ...
            for about_retry in range(3):
                ...
                if response_about.status_code == 200:
                    ...
                    if isinstance(about_data, dict):
                        ...
                        new_page = _page_type_from_payload(about_data)
                        if new_continue:
...
def run(domain=None, proxy=None, show_header=True):
    ...
    if show_header:
        print("\n" + "=" * 50)
        print("[*] 注册 (反风控版)")
        domain_label = "auto-random" if str(_current_domain).strip().lower() in {"", "auto", "random"} else _current_domain
    ...
    selected_domain = email.rpartition("@")[2]
    print(f"[*] 本次邮箱域名: {selected_domain}")
...
def _quick_preflight(proxy: Optional[str] = None) -> bool:
    ...
    try:
        r = std_requests.get(
            _resolve_verify_api_url(_current_domain),
            timeout=15,
...
def _run_one(idx, total, domain=None, proxy=None):
    try:
        with _print_lock:
            domain_label = domain or _current_domain
            if str(domain_label).strip().lower() in {"", "auto", "random"}:
                domain_label = "auto-random"
            print(f"\n{'=' * 60}")
...
def run_batch(total_accounts: int = 1, max_workers: int = 1, domain=None, proxy=None):
    ...
    print(f"  域名: {domain_label}")
    print(f"  代理: {proxy or '不使用代理'}")
    print(f"  验证码接口: {_resolve_verify_api_url(domain or _current_domain)}")
...
def _run_smoke_test() -> None:
    print("[Smoke] Starting Register Full smoke test.")
    print(f"[Smoke] Bundle dir : {APP_BUNDLE_DIR}")
    print(f"[Smoke] Runtime dir: {APP_RUNTIME_DIR}")
    print(f"[Smoke] Token dir  : {TOKEN_JSON_DIR}")

    with tempfile.TemporaryDirectory(prefix="register-full-smoke-") as temp_dir:
        marker_path = os.path.join(temp_dir, "artifacts", "marker.txt")
        _ensure_parent_dir(marker_path)
        with open(marker_path, "w", encoding="utf-8") as handle:
            handle.write("ok\n")

    test_domain = next(iter(VERIFY_API_URL_BY_DOMAIN))
    resolved_api = _resolve_verify_api_url(test_domain)
    if not resolved_api.startswith("https://"):
        raise RuntimeError(f"Unexpected verify api url: {resolved_api}")
    ...
    sentinel_config = sentinel._get_config()
    if not isinstance(sentinel_config, list) or len(sentinel_config) < 5:
        raise RuntimeError("Sentinel token generator did not produce a config.")

    print("[Smoke] Register Full smoke test passed.")
...
def main():
    ...
    default_domain_label = "auto-random" if str(_current_domain).strip().lower() in {"", "auto", "random"} else _current_domain
    print(f"  默认域名: {default_domain_label}")
    print("=" * 60)
