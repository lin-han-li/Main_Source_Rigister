...
set "DISPLAY_TITLE=OpenAI Register Only"
title %DISPLAY_TITLE%
...
%PYTHON_CMD% register_only.py %SCRIPT_ARGS%
...
if "%EXIT_CODE%"=="0" (
    ...
    echo [ERROR] register_only.py exited with code %EXIT_CODE%.
)
...
echo.
echo Notes:
echo   - No argument: use register_only.py interactive prompts.
echo   - First numeric argument is converted to "-n COUNT" for compatibility.
echo   - All other options are passed through to register_only.py.
exit /b 0
